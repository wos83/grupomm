...
type
  ...
  private
    ...
    procedure insereDadosHistoricoPosicao(AJsonData: string);
    { Private declarations }
  public
    FLogin: TFDMemTable;
    FPositionLast: TFDMemTable;
    FPositionHistory: TFDMemTable;
    ...
    function doPositionHistory(AToken: string; AEquipamentId: integer; AStartDate, AStartTime, AEndDate, AEndTime: string; APage: integer; out ATotalPage: integer): boolean;
    { Public declarations }
  end;
...
const
  ...
  cAPI_PositionHistoryPeriod = '/position/history/%d/period/%s %s/%s %s?page=%d';
...
function TDM.doPositionHistory(AToken: string; AEquipamentId: integer; AStartDate, AStartTime, AEndDate, AEndTime: string; APage: integer; out ATotalPage: integer): boolean;
var
  FQry: TFDQuery;
  LSQL: string;
...
begin
  ...
  if (AEquipamentId.ToString = EmptyStr) then
  begin
    LError := 'Código do Equipamento em Branco.';
    Exit;
  end;

  if (AStartDate = EmptyStr) then
  begin
    LError := 'Data Inícial em Branco.';
    Exit;
  end;

  if (AStartDate = EmptyStr) then
  begin
    LError := 'Hora Inícial em Branco.';
    Exit;
  end;

  if (AEndDate = EmptyStr) then
  begin
    LError := 'Data Final em Branco.';
    Exit;
  end;

  if (AEndDate = EmptyStr) then
  begin
    LError := 'Hora Final em Branco.';
    Exit;
  end;

  if (APage.ToString = EmptyStr) then
  begin
    APage := 1;
  end;
  ...
  try
    ...
    LRESTResponseDataSetAdapter.DataSet := FPositionHistory;
    LRESTResponseDataSetAdapter.Response := LRESTResponse;
    LRESTResponseDataSetAdapter.NestedElements := True;
    ...
    // , LRESTResponse.Content);
    {$ENDIF}
    if FPositionHistory.Active then
    begin
      if not FPositionHistory.IsEmpty then
      begin
        LJsonResult := LRESTResponse.Content;
        LTotal := CountWords(cID, LJsonResult);
        ...
        LSQL := //
           ...
           'AND EQUIPMENT_ID = ' + AEquipamentId.ToString + sLineBreak + //
           'AND EVENT_DATE BETWEEN ' + QuotedStr(AStartDate + ' ' + AStartTime) + sLineBreak + //
           'AND ' + QuotedStr(AEndDate + ' ' + AEndTime) + sLineBreak + //
           '';
              ...
              TThread.Queue(nil,
                ...
                begin
                  TThread.Synchronize(nil,
                    procedure
                    begin
                      insereDadosHistoricoPosicao(LJsonData);
                    end);
                end);
            end;
          end;
        end;
        // Toast('Histórico de Posições Atualizada!', 3);
        Result := True;
      end;
    end;
...
begin
  ...
  FPositionHistory := TFDMemTable.Create(nil);
...
begin
  ...
  FreeAndNil(FPositionHistory);
  FreeAndNil(FGeocoder);
  {$ENDIF}
  {$IFDEF ANDROID}
  FLogin.DisposeOf;
  FPositionLast.DisposeOf;
  FPositionHistory.DisposeOf;
  FGeocoder.DisposeOf;
  {$ENDIF}
end;
...
begin
  ...
  begin
    ...
    // begin
    // LLocation.Latitude := StrToFloatDef(StringReplace(LJsonObject.Values['latitude'].Value, '.', ',', [rfReplaceAll]), 0);
    // LLocation.Longitude := StrToFloatDef(StringReplace(LJsonObject.Values['longitude'].Value, '.', ',', [rfReplaceAll]), 0);
    //
    ...
    // LAddress := //
    // FGeoInfo.AdminArea + //
    // ',' + FGeoInfo.CountryCode + //
    // ',' + FGeoInfo.CountryName + //
    // ',' + FGeoInfo.FeatureName + //
    // ',' + FGeoInfo.Locality + //
    // ',' + FGeoInfo.PostalCode + //
    // ',' + FGeoInfo.SubAdminArea + //
    // ',' + FGeoInfo.SubLocality + //
    // ',' + FGeoInfo.SubThoroughfare + //
    // ',' + FGeoInfo.Thoroughfare + //
    // '';
    // end;
...
procedure TDM.insereDadosHistoricoPosicao(AJsonData: string);
var
  LJsonObject: TJSONObject;
  LJsonValue: TJSONValue;
...
begin
  ...
  if (LId = 0) then
  begin
    LogApp('APP: Historico Posicao nao conseguiu identificar o ID', EmptyStr, EmptyStr);
    Exit;
  end;
  ...
  LSQL := //
     ...
     'FROM POSITIONS_HISTORY' + sLineBreak + //
     'WHERE 1 = 1' + sLineBreak + //
     'AND ID_POSITION_HISTORY = ' + IntToStr(LId) + sLineBreak + //
     '';
  ...
  try
    ...
    try
      ...
      begin
        LogApp( //
           'TDM.insereDadosHistoricoPosicao' //
           , E.ClassName + '. ' + E.Message //
           , LSQL);
      end;
    end;
  ...
  begin
    ...
    LSQL := //
       ...
       IntToStr(FUser.Id) + ' -- ID_USER' + sLineBreak + //
       ',' + IntToStr(LId) + ' -- ,ID_POSITION_HISTORY' + sLineBreak + //
       ',' + QuotedStr(LType) + ' -- ,DS_TYPE' + sLineBreak + //
       ',' + IntToStr(LEquipmentId) + ' -- ,EQUIPMENT_ID' + sLineBreak + //
       ',' + IntToStr(LEquipmentModelId) + ' -- ,EQUIPMENT_MODEL_ID' + sLineBreak + //
       ',' + QuotedStr(LEquipmentModelName) + ' -- ,EQUIPMENT_MODEL_NAME' + sLineBreak + //
