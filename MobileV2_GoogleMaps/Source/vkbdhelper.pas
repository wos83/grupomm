...
type
  ...
  protected
    ...
    function FindOffsettedScrollBox(AControl: TControl; VKBounds: TRectF): TCustomScrollBox;
    ...
    procedure ScrollIntoControl(AScrollBox: TCustomScrollBox; FocusedControl: TControl);

    procedure CalcScrollBoxContentBounds(Sender: TObject; var ContentBounds: TRectF);
    ...
    procedure LastControlNotification(AComponent: TComponent; Operation: TOperation);
...
begin
  Result := (AControl is TRectangle) and (AControl.Parent is TCommonCustomForm) and
    (TRectangle(AControl).Align = TAlignLayout.Contents);
end;
...
{ TVKStateHandler }
procedure TVKStateHandler.CalcScrollBoxContentBounds(Sender: TObject; var ContentBounds: TRectF);
...
begin
  ...
  begin
    ContentBounds.Bottom := Max(ContentBounds.Bottom, 2 * TControl(Sender).Height);
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FVKBounds.BottomRight := tmpParentForm.ScreenToClient(FVKBounds.BottomRight);
      end;
...
function TVKStateHandler.FindOffsettedScrollBox(AControl: TControl; VKBounds: TRectF): TCustomScrollBox;
var
...
begin
  ...
  begin
    if (tmpParent is TCustomScrollBox) then
      if (TCustomScrollBox(tmpParent).AbsoluteRect.Top + FocusedControlRect.Height) < VKBounds.Top then
      begin
...
function TVKStateHandler.FindOrCreateCommonLayout(AForm: TCommonCustomForm): TLayout;
begin
...
function TVKStateHandler.GetVKBounds(var ARect: TRect; NewFormSize: TSize): Boolean;
var
...
procedure TVKStateHandler.LastControlNotification(AComponent: TComponent; Operation: TOperation);
var
  tmpControl: TControl;
begin
  if Assigned(FLastControl) then
    ...
    begin
      ...
      begin
        TCustomScrollBox(FLastControl).OnCalcContentBounds := FOldCalcContentBounds;
  ...
  begin
    ...
    begin
      ...
      TControl(AComponent).SetBounds(0, 0, TCommonCustomForm(TControl(AComponent).Parent).ClientWidth,
    ...
    begin
      ...
      TCustomScrollBox(AComponent).OnCalcContentBounds := CalcScrollBoxContentBounds;
...
procedure TVKStateHandler.MoveCtrls(AOldParent: TCommonCustomForm; ANewParent: TFmxObject);
var
  i: Integer;
  AChild: TFmxObject;
begin
  ...
  begin
    ...
    begin
      if AChild.Parent = AOldParent then
        if (not ControlIsBackground(AChild)) and (not ControlIsIgnored(AChild)) then
        begin
...
procedure TVKStateHandler.OffsetForControl(AMovedLayout: TLayout; FocusedControl: TControl);
var
  FocusedRect: TRectF;
begin
  ...
  if FocusedRect.IntersectsWith(FVKBounds) then
    AMovedLayout.SetBounds(AMovedLayout.Position.X, FVKBounds.Top - FocusedRect.Bottom, AMovedLayout.Width,
      AMovedLayout.Height);
end;

procedure TVKStateHandler.ScrollIntoControl(AScrollBox: TCustomScrollBox; FocusedControl: TControl);
var
  FocusRect: TRectF;
begin
  ...
  if (FocusRect.IntersectsWith(FVKBounds)) and (FocusRect.Bottom > FVKBounds.Top) then
  begin
    AScrollBox.ViewportPosition := PointF(AScrollBox.ViewportPosition.X, FocusRect.Bottom - FVKBounds.Top);
  end;
end;
